...
public class Heap<E> {
	...
	private void bubbleUp(int index) {
		...
		if (index == 1
				|| heapArray.get(parentIndex).getPriority() >= heapArray.get(
						index).getPriority()) {
			return;
		}
	...
	private void sinkDown(int index) {
		...
		if (rightChild > heapSize) {
			...
			if (heapArray.get(rightChild).getPriority() > heapArray.get(
					leftChild).getPriority()) {
				maxIndex = rightChild;
			} else {
				maxIndex = leftChild;
			}
		...
		if (heapArray.get(index).getPriority() > heapArray.get(maxIndex)
				.getPriority()) {
			// do nothing
			return;
		} else {
